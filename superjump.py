...
def main():
    ...
    while True:
        ...
        if state == "start":
            ...
            title = font_giant.render(GAME_TITLE, True, SCORE_COL)
            
            # Create a glow effect
            alpha_glow = int(128 + math.sin(t * 0.005) * 127)
            glow_surf = font_giant.render(GAME_TITLE, True, (255,200,0))
            glow_surf.set_alpha(alpha_glow)
            
            draw_text_shadow(screen, GAME_TITLE, font_giant, SCORE_COL, SCREEN_W//2 - title.get_width()//2, title_y, shadow=(0,0,0))
